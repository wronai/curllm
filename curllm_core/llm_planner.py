...
async def generate_action(llm: Any, instruction: str, page_context: Dict, step: int, run_logger: RunLogger | None = None, max_chars: int = 8000, growth_per_step: int = 2000, max_cap: int = 20000) -> Dict:
    ...
    tools_desc = (
        ...
        "- products.extract(args: {max_items?: number}): returns {products: [{name, price, url}]} [RECOMMENDED - uses dynamic pattern detection]\n"
        "- products.heuristics(args: {threshold?: number}): returns {products: [{name, price, url}]} [DEPRECATED - redirects to products.extract]\n"
