...
async def generate_action(llm: Any, instruction: str, page_context: Dict, step: int, run_logger: RunLogger | None = None, max_chars: int = 8000, growth_per_step: int = 2000, max_cap: int = 20000) -> Dict:
    ...
    if "product" in (instruction or "").lower() or "cen" in (instruction or "").lower():
        product_context = (
            ...
            "✅ You MUST use the products.heuristics TOOL to extract products:\n\n"
            "```json\n"
            "{\n"
            "  \"type\": \"tool\",\n"
            "  \"tool_name\": \"products.heuristics\",\n"
            "  \"args\": {},\n"
            "  \"reason\": \"Extracting products with prices using heuristics\"\n"
            "}\n"
            "```\n\n"
            ...
            "2. If page shows products: call products.heuristics tool\n"
            "3. If tool_history contains products from products.heuristics: return type=\"complete\" with extracted_data={\"products\": products}\n"
